(function($) {
            ...
            return $.ajax({
                ...
                success: function(result) {
                    if (result && result.hasOwnProperty("ret") && result.hasOwnProperty("errcode")) {
                        if (result.ret === 1 && result.errcode === -1) {
    ...
    $saas.clipboardData = function(text) {
    ...
    $saas.expressCompany = function() {
        return [
            { id: "1", name: "顺丰", code: "shunfeng" },
            { id: "2", name: "圆通", code: "yuantong" },
            { id: "3", name: "中通", code: "zhongtong" },
            { id: "4", name: "韵达", code: "yunda" },
            { id: "5", name: "EMS", code: "ems" }
        ];
    };

    $saas.thirdWayPlatformType = {
        getAll: function() {
            return [
                { id: "1", name: "微信" },
                { id: "2", name: "天猫" },
        ...
        getNameById: function(id) {
            var model = $.grep($saas.thirdWayPlatformType.getAll(), function(item) {
                return item.id == id;
            });
            return model.length > 0 ? model[0].name : undefined;
        }
    };
    ...
    $saas.payType = {
        ...
        getNameById: function(id) {
            var model = $.grep($saas.payType.getAll(), function (item) {
                return item.id == id;
            });
            return model.length > 0 ? model[0].name : undefined;
        }
